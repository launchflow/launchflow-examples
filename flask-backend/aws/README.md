...
An example Flask backend that deploys to [ECS Fargate on AWS](https://aws.amazon.com/fargate/) using [LaunchFlow](https://launchflow.com/).
...
<strong>NOTE:</strong> The AWS infrastructure is defined in [infra.py](/flask-backend/aws/app/infra.py)
...
### Automatically <strong>build</strong> and <strong>deploy</strong> the Flask application to AWS ECS Fargate
